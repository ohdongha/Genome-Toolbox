...
synopsis = "\n\
create_hist_less_or_equal.py <table.txt> <N> <bin_size> <output.hist>\n\n\
 ...
 - count x in Nth_bin< x <=(N+1)th_bin (except for the 1st bin);\n\
 - to count x in Nth_bin<= x <(N+1)th_bin, use the original 'create_hist.py';\n\
 - ignore non-numerical values from the input.\n\
by ohdongha@gmail.com ver0.0 20201125 modified from 'count_hist.py' ver0.2.4\n"

#version_history
#20201125 ver 0.0 # modified from "count_hist.py" for use in special occasions ...
...
#		Value_dict[int(Value / bin_size)] = Value_dict.get(int(Value / bin_size), 0) + 1  # original line from "create_hist.py"
		if Value == 0.0:
			Value_dict[0] = Value_dict.get(0, 0) + 1
		elif int(Value / bin_size) == (Value / bin_size):
			Value_dict[int(Value / bin_size) - 1] = Value_dict.get(int(Value / bin_size) - 1, 0) + 1
		else:
